...
public final class YsmGeoPlayerRenderer extends GeoObjectRenderer<YsmGeoAnimatablePlayer, YsmRenderContext, GeoRenderState.Impl> {
    ...
    public void addRenderData(YsmGeoAnimatablePlayer animatable, YsmRenderContext relatedObject, GeoRenderState.Impl renderState, float partialTick) {
        ...
        renderState.addGeckolibData(BODY_YAW, vanillaState.bodyRot);
        ...
        renderState.addGeckolibData(DataTickets.ENTITY_YAW, relatedObject.player().getYHeadRot());
        renderState.addGeckolibData(DataTickets.ENTITY_PITCH, relatedObject.player().getXRot());

        renderState.addGeckolibData(DataTickets.VELOCITY, relatedObject.player().getDeltaMovement());
        renderState.addGeckolibData(DataTickets.ELYTRA_ROTATION, new Vec3(0.0, 0.0, poseSnapshot.elytraRoll()));
        renderState.addGeckolibData(DataTickets.SWINGING_ARM, relatedObject.player().swinging);
        renderState.addGeckolibData(DataTickets.IS_LEFT_HANDED, relatedObject.player().getMainArm() != net.minecraft.world.entity.HumanoidArm.RIGHT);
    }
    ...
    @Override
    public void preRenderPass(RenderPassInfo<GeoRenderState.Impl> renderPassInfo, SubmitNodeCollector renderTasks) {
    ...
    public void adjustRenderPose(RenderPassInfo<GeoRenderState.Impl> renderPassInfo) {
        ...
        renderPassInfo.poseStack().mulPose(Axis.YP.rotationDegrees(180.0F - bodyYaw));

        float deathTime = renderPassInfo.renderState().getOrDefaultGeckolibData(DEATH_TIME, 0.0F);
        if (deathTime > 0.0F) {
            float deathRotation = Math.min(Mth.sqrt((deathTime - 1.0F) / 20.0F * 1.6F), 1.0F) * 90.0F;
            renderPassInfo.poseStack().mulPose(Axis.ZP.rotationDegrees(deathRotation));
        }
    }
    ...
    private static void applyHeadTracking(BoneSnapshots snapshots, YsmPoseSnapshot pose) {
        ...
        float yawRadians = (float) Math.toRadians(Mth.clamp(pose.relativeHeadYaw(), -75.0f, 75.0f)) * 0.85f;
        float pitchRadians = (float) Math.toRadians(Mth.clamp(pose.headPitch(), -60.0f, 60.0f)) * 0.85f;
